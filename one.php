...
<p style="margin-left:400px;margin-top:10px;font-size:30px;">YOU'R ATTENDENCE IS</p>
<hr>
<?php
session_start();
...
$t="SELECT column_name FROM information_schema.columns WHERE table_name = '$subject' and column_name!='roll_no' and table_schema = '$b'";
$result1=mysqli_query($bd1,$t);
$i=0;
while($row1=mysqli_fetch_array($result1))
{
	echo'<table style="font-size:24px;margin-left:500px; "><tr>';
	echo '<td>',$row1[$i],'</td>';
	$result2=mysqli_query($bd1,"select * from `$subject` where roll_no='$roll_no'");
	$row2=mysqli_fetch_array($result2);
	echo'<td>',$row2[$row1[$i]],'</td>';
}
echo'</tr></table>';
